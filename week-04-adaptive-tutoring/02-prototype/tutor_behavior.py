def detect_scenario(user_question):
    ...
    if any(keyword in question for keyword in direct_solution_keywords):
...
Stay close to retrieved course materials.
Do not add your own examples when the retrieved course materials already answer the question.
...
- Do not add outside examples, analogies, or extra scenarios when the retrieved course materials already answer the question.
- Only use examples that appear in the retrieved course materials.
- If the retrieved course materials do not include an example, explain the concept without inventing a new example.
...
- Keep the response concise, usually 1-2 short paragraphs.
